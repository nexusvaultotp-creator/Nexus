...
app.post('/api/request-otp', async (req, res) => {
  ...
  else res.status(500).json({ error: 'Failed to send OTP. Check logs.' });
});
...
// --- API fetching with jitter to avoid 430 error ---
let lastBtcFetch = 0;
async function fetchBTCTransactions() {
  const now = Date.now();
  if (now - lastBtcFetch < 120000) return []; // 2 min cooldown
  lastBtcFetch = now;
  try {
...
// Poll every 2 minutes with random offset (90-150s) to avoid 430
cron.schedule('*/2 * * * *', () => {
  const jitter = Math.random() * 60000; // 0-60s extra delay
  setTimeout(() => matchIncomingTransactions(), jitter);
});
