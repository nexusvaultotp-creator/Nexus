...
async function fetchBTCTransactions() {
  try {
    const url = `https://api.blockchair.com/bitcoin/dashboards/address/${BTC_ADDRESS}?transaction_details=true&limit=10`;
  ...
  } catch(e) { console.error('BTC error:', e.message); return []; }
}
...
cron.schedule('*/30 * * * * *', matchIncomingTransactions);
